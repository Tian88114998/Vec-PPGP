...
res_vec = neg_vecchia_marginal_log_likelihood(params = c(gamma, nu), nu = 1, nugget_est = TRUE, x = ordered_sample, NNmatrix = nnmatrix, y = Yo, trend = matrix(1, nrow = n), kernel_type = "matern_5_2", alpha = 1.9, zero_mean = "No")
...
# the input type of kernel_type is wrong. It only takes integer, not string
log_nu = log(nu)
res = RobustGaSP::log_marginal_lik_ppgasp(param = c(log_inv_gamma, log_nu), nugget = 2, nugget_est = TRUE,
...
p = 1
k = 5
temp = generate_data(n = n, p = p, k = k, gamma_range = c(0, 2), sigma2_range = c(0,0.5), nugget_range = c(0,1))
Xo = temp[[1]]
Yo = temp[[2]]

n_param = 100
# tau and eta is from -1 to 1
tau = seq(-2, 2, length.out = n_param)
eta = seq(-2, 2, length.out = n_param)
# nu and gamma are from 0.36 to 2.718
nu = exp(tau)
gamma = 1/exp(eta)

m_list = round(c(n-1, n/2, n/4, n/6, n/8, n/10))

matrix_list = lapply(1:length(m_list), function(x) matrix(0, nrow = 100, ncol = 100))
...
for (i in 1:length(m_list)){
  ...
  for (j in 1:n_param){
    for (k in 1:n_param){
      gamma_j = gamma[j]
      nu_k = nu[k]
      res = neg_vecchia_marginal_log_likelihood(params = c(gamma_j, nu_k), x =ordered_sample,NNmatrix = nnmatrix, y = ordered_response, kernel_type = "pow_exp", nu = 0, nugget_est = TRUE, alpha = 1.9)
      matrix_list[[i]][j,k] = res
    }
  }
}

z_df <- expand.grid(X = eta, Y = tau)
z_df$Z <- as.vector(t(matrix_list[[1]]))

ggplot(z_df, aes(X, Y, fill = Z)) +
  geom_tile() +
  scale_fill_viridis_c() +
  labs(title = "Heatmap with ggplot2", fill = "Z") +
  theme_minimal()

filled.contour(eta, tau, t(matrix_list[[1]]),
...
log_nu = log(nu)
log_like_deriv = RobustGaSP::log_marginal_lik_deriv_ppgasp(param = c(log_inv_gamma, log_nu), nugget = nu, nugget_est = TRUE, R0 = R0, X = matrix(1, nrow = n), zero_mean = "Yes",output = Yo, kernel_type = c(3,3), alpha =c(1.9,1.9))

temp = neg_vecchia_marginal_log_likelihood_deriv(params = c(gamma, nu), nu = nu, nugget_est = TRUE, x = ordered_sample, NNmatrix = nnmatrix, y = Yo, trend = matrix(1, nrow = n), zero_mean = "Yes", kernel_type = "matern_5_2", alpha = 1.9)
```

```{r deriv_with_mean}
# check the derivative with mean
set.seed(2001)
temp = generate_data(n = 200, p = 5, k = 5, gamma_range, sigma2_range, nugget_range)
n = 200
p = 5
Xo = temp[[1]]
Yo = temp[[2]]
...
neg_vecchia_marginal_log_likelihood_deriv(c(gamma,nu), nu = nu, nugget_est = TRUE, x = ordered_sample, NNmatrix = nnmatrix, y = Yo, trend = matrix(rep(1, n)), kernel_type = "pow_exp", alpha = 1.9, zero_mean = "Yes")

R0 = generate_R0(ordered_sample, ordered_sample)
log_inv_gamma = log(1/gamma)

RobustGaSP::log_marginal_lik_deriv_ppgasp(param = c(log_inv_gamma, nu), nugget = nu, nugget_est = TRUE, R0 = R0, X = matrix(1, ncol = n), zero_mean = "No",output = Yo, kernel_type = c(1,1,1,1,1), alpha =c(1.9,1.9,1.9,1.9,1.9))
```
...
set.seed(2001)
start_time = Sys.time()
res = optim(par = c(gamma), fn = neg_vecchia_marginal_log_likelihood, gr = neg_vecchia_marginal_log_likelihood_deriv, lower = rep(10e-9, 6), nu = nu, nugget_est = FALSE, x = ordered_sample, NNmatrix = nnmatrix, y = Yo, alpha = 1.9, method = "BFGS")
end_time = Sys.time()
end_time - start_time

start_time <- Sys.time()
m.ppgasp=ppgasp(design=ordered_sample, response=Yo, nugget = nu, nugget.est = FALSE, zero.mean = "Yes", kernel_type = "pow_exp")
end_time <- Sys.time()
show(m.ppgasp)
...
set.seed(2001)
start_time = Sys.time()
res = optim(par = c(gamma), fn = neg_vecchia_marginal_log_likelihood, gr = neg_vecchia_marginal_log_likelihood_deriv, lower = rep(10e-9, 6), nu = 1, nugget_est = FALSE, x = X_train_ordered, NNmatrix = nnmatrix_train, y = Y_train_ordered, kernel_type = "matern_5_2", alpha = 1.9, method = "L-BFGS-B")
end_time = Sys.time()
end_time - start_time
# 21.89422 secs
...
## Compare to PPGP 
start_time <- Sys.time()
m.ppgasp=ppgasp(design=X_train_ordered, response=Y_train_ordered, nugget = 1, nugget.est = F, kernel_type = "matern_5_2", zero.mean = "No", optimization = 'lbfgs')
end_time <- Sys.time()
end_time - start_time
# Time difference without the nugget estimation is 37.43615 secs
# With nugget estimation is 1.182473 mins

list = predict(x = X_train_ordered, xp = X_test_ordered, NNmatrix = nnmatrix_all, y = Y_train_ordered, gamma = res$par, nu = 1, kernel_type = "matern_5_2", alpha = 1.9, q = qnorm(0.975, 0, 1))
...
set.seed(2001)
start_time = Sys.time()
res = optim(par = c(gamma, nu), fn = neg_vecchia_marginal_log_likelihood, gr = neg_vecchia_marginal_log_likelihood_deriv, lower = rep(10e-9, 6), nu = nu, nugget_est = TRUE, x = X_train_ordered, NNmatrix = nnmatrix_train, y = Y_train_ordered, kernel_type = "matern_5_2",alpha = 1.9, method = "L-BFGS-B")
end_time = Sys.time()
end_time - start_time
# Time difference of 47.37236 secs

list = predict(x = X_train_ordered, xp = X_test_ordered, NNmatrix = nnmatrix_all, y = Y_train_ordered, gamma = res$par[-length(res$par)], nu = res$par[length(res$par)], kernel_type = "matern_5_2", alpha = 1.9, q = qnorm(0.975, 0, 1))
...
## Compare to PPGP 
start_time <- Sys.time()
m.ppgasp=ppgasp(design=X_train_ordered, response=Y_train_ordered, nugget.est = T, kernel_type = "matern_5_2", zero.mean = "No", optimization = 'lbfgs')
end_time <- Sys.time()
end_time - start_time
...
set.seed(2001)
start_time = Sys.time()
res = optim(par = c(gamma,nu), fn = neg_vecchia_marginal_log_likelihood, gr = neg_vecchia_marginal_log_likelihood_deriv, lower = rep(10e-9, 6), nu = 1, nugget_est = TRUE, x = as.matrix(X_train_ordered), NNmatrix = nnmatrix_train, y = as.matrix(Y_train_ordered_log_1[,-index_all_zero]), alpha = 1.9, method = "L-BFGS-B")
end_time = Sys.time()
end_time - start_time
# Time difference of 359.4098 s for n = 50
# Time difference of 4.5275 s

list = predict(x = as.matrix(X_train_ordered), xp = as.matrix(X_test_ordered), NNmatrix = nnmatrix_all, y = as.matrix(Y_train_ordered_log_1[,-index_all_zero]), gamma = res$par[-length(res$par)], nu = res$par[length(res$par)], alpha = 1.9, q = qnorm(0.975, 0, 1))
